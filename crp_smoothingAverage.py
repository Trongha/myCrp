...
def makeTestFeature(trainSet, testSet, dim=3, tau=2, epsilon=0.0055, lambd=3, percent=1, distNorm = 1, pathFolder = "result/", formatSave = ".png", trainSetID = None):
	...
	title = "IDtrain_" + str(trainSetID) + "_" + str(numSample) + " - dim_" + str(dim) + " - epsil_" + str(epsilon)
	print("\n------------------------------------------------", title,"------------------------------------------------\n")
...
if (__name__ == "__main__"):
	...
	pathFolder = "10012019SmoothingAverage/"
	...
	for sizeWindow in range(1,15):
		pathFolder1 = "{}WindowSize_{}".format(pathFolder, sizeWindow)
		allTrainSetAverageSmoothing = [myCrpFunctions.smoothingMovingAverage(trainSet, sizeWindow) for trainSet in allTrainSet]
		testSetAverage = myCrpFunctions.smoothingMovingAverage(testSet, sizeWindow)

		for epsilon in [0.0085, 0.005]:
			pathFolder2 = '{}Epsilon_{}/'.format(pathFolder1, epsilon)
			for i in range(0, len(allTrainSetAverageSmoothing)):
				if (len(allTrainSetAverageSmoothing[i]) > myDim*tau):
					makeTestFeatureZoom(allTrainSetAverageSmoothing[i], testSetAverage, 
