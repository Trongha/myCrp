				...
				if (i-start+1 > lambd):
					...
					for j in range(start, i+1, 1):
						...
						if (i_row < high_r1):
							...
							diagonalsMatrix[i_row][j] = (i_row - high_r1 + j)			
			i+=1
		# Nếu hàng có dự đoán thì sẽ giữ lại
		if (havePredict == 1):

			#Xử lý những đường chéo ở góc, hiện tại index của nó chưa trùng với index predict của train,
			# phải cho nó thụt về sau
			# 
			if (i_row < high_r1):
				#print("\n-------------i_row < high_r1-----------------\n")
				for i in range(high_r1 - i_row - 1):
					diagonalsMatrix[i_row] = np.insert(diagonalsMatrix[i_row], 0, -2)

			indexHavePredictMatrix.append(diagonalsMatrix[i_row])

	#Mảng quy đổi về index của dữ liệu test từ index của statePhrase
	
	f_crp = myCrpFunctions.crossRecurrencePlots("CRP2", indexHavePredictMatrix, dotSize = 0.2)
