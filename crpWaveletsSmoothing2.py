...
def predict_diagonal(trainSet, testSet, dim=5, tau=2, epsilon=0.7, lambd=3, percent=0.6, distNorm = 1,
	...
	percentIndext = len(shapePredict)/len(testSet)
	...
	for i in range(len(indexSampleOrigin)):
		...
		for j in range(len(indexSampleOrigin[i])):
			if (indexSampleOrigin[i][j] < 0): 
				...
				try:
					indexx = int(indexSampleOrigin[i][j]*percentIndext)
					shapePredict[indexx] = shape
					shapePredict[indexx+1] = shape
				except (IndexError):
...
if (__name__ == "__main__"):
	...
	formatSave = ".png"
	epsilon = 0.009
	pathFolder = "1401_waveletSmoothing/"
	...
	types = ['sym2', 'sym3', 'sym4', 'sym5', 'sym6', 'sym7', 'sym8', 'sym9', 'sym10', 'sym11', 'sym12', 'sym13', 'sym14', 'sym15', 'sym16']


	for type in types:
		pathFolder1 = "{}Type_{}".format(pathFolder, type)
		allSmoothingTrainSet = [myCrpFunctions.waveletsmoothing(trainSet, type = type) for trainSet in allTrainSet]
		smoothingTestSet = myCrpFunctions.waveletsmoothing(testSet, type = type)

		for epsilon in [0.0085, 0.005]:
			pathFolder2 = '{}-Epsilon_{}/'.format(pathFolder1, epsilon)
			for i in range(0, len(allSmoothingTrainSet)):
				if (len(allSmoothingTrainSet[i]) > myDim*tau):
					makeTestFeatureZoom(allSmoothingTrainSet[i], smoothingTestSet, 
