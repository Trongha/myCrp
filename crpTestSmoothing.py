...
if (__name__ == "__main__"):
	...
	allTrainSet1 = [savgol_filter(set, 7, 5) for set in allTrainSet if len(set) > 11]

	testSet = savgol_filter(testSet, 11, 5)

	pathFolder1 = "{}Savitzky–Golay".format(pathFolder)
	for i in range(0, len(allTrainSet1)):
		makeTestFeatureZoom(allTrainSet1[i], testSet, 
											dim= myDim, tau= tau, epsilon= epsilon, lambd= myLambd, percent=1, distNorm = distNorm, 
											pathFolder = pathFolder1, formatSave = ".png", trainSetID = i)
	checkRecall(pathFolder1)
	refreshPredict()
	...
	for window in windows:
		for window_len in range(4, 12):
			pathFolder1 = '{}{}_{}/'.format(pathFolder, window, window_len)
			allTrainSetSmoothed = []
			for i in range(len(allTrainSet)):
				if (len(allTrainSet[i]) >= window_len):
					allTrainSetSmoothed.append(smoothing1.smooth(np.array(allTrainSet[i]), window_len = window_len, window = "hamming"))

			testSet = smoothing1.smooth(np.array(testSet), window_len = window_len, window = "hamming")		
			
			print("len(trainSet): ", len(allTrainSetSmoothed))
			print("len(testSet): ", len(testSet))
			for epsilon in [0.01, 0.009, 0.0085, 0.007, 0.0055, 0.0045]:
				pathFolder2 = '{}Epsilon_{}/'.format(pathFolder1, epsilon)
				for i in range(0, len(allTrainSetSmoothed)):
					if (len(allTrainSetSmoothed[i]) > myDim*tau):
						makeTestFeatureZoom(allTrainSetSmoothed[i], testSet, 
										dim= myDim, tau= tau, epsilon= epsilon, lambd= myLambd, percent=1, distNorm = distNorm, 
										pathFolder = pathFolder2, formatSave = ".png", trainSetID = i)
				checkRecall(pathFolder2)
				refreshPredict()
	'''